...
import { BlogService, Blog } from "../backend/blog.service";
import { AuthService } from "../backend/auth.service";
import LogoutButton from "./logout-form";
import { useState, useEffect } from "react";
import { Calendar, User, Eye, Edit3, Trash2, Plus, BookOpen, Sparkles } from "lucide-react";
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 flex items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground animate-pulse">Loading amazing blogs...</p>
      </div>
    </div>
  );
  if (error) return (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-2 text-lg">Discover amazing stories and insights</p>
                </div>
              </div>
            </div>
        ...
        {blogs?.length ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {blogs.map((blog: Blog, index: number) => (
              <Card key={blog.id} className="group relative overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-card via-card to-card/50 hover:scale-[1.02] animate-in fade-in-0 slide-in-from-bottom-4" style={{animationDelay: `${index * 100}ms`}}>
                {/* Gradient overlay */}
                  ...
                  <div className="flex flex-col gap-2 mt-4 align-bottom justify-end">
                    <div className="grid grid-cols-1 gap-2">
                      <Link href={`/view/${blog.id}`}>
                        <Button variant="secondary" size="sm" className="w-full group/btn hover:bg-primary hover:text-primary-foreground transition-all duration-300">
                    ...
                    {currentUserId && blog.user_id === currentUserId && (
                      <div className="grid grid-cols-2 gap-2">
                        <Link href={`/edit/${blog.id}`}>
                          <Button variant="outline" size="sm" className="w-full hover:bg-accent hover:text-accent-foreground transition-all duration-300">
          ...
          <div className="text-center py-16 space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-semibold text-muted-foreground">No blogs yet</h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                Be the first to share your thoughts and create an amazing blog post!
              </p>
            </div>
            <Link href="/create">
              <Button size="lg" className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-lg hover:shadow-xl transition-all duration-300">
